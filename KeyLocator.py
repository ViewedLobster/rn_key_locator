...
class KeyLocator:
    ...
    def main(self):
        ...
        while not done:
            ...
            if not lastState == pinKeyValue:
                ...
                self.doCurl(keyValue = pinKeyValue, doorValue = pinDoorValue, messageType = "changed")
                lastTime = time.time()
            else:
                currentTime = time.time()
                if currentTime - lastTime > 1800.:
                    self.doCurl(keyValue = pinKeyValue, doorValue = pinDoorValue, messageType = "ping")
            ...
            if pinDoorValue and not pinKeyValue and not sentSMS:
                ...
                self.doCurl(keyValue = pinKeyValue, doorValue = pinDoorValue, messageType = "key_alarm_alert_emergency")
    ...
    def doSMS(self):
        ...
        pycurl_connect.setopt(pycurl.POSTFIELDS, "message=brandlarmet har inte g\xc3tt&send_to_self=1")
